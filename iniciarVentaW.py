...
ORDER BY
    ...
    FEC;
        ...
        """
            ...
            # 🚀 OPTIMIZACIÓN: Pre-procesar nombres de widgets para evitar llamadas repetidas a normalizar_texto
            processed_widgets = []
            for vend_ui in widgets.keys():
                norm_ui = normalizar_texto(vend_ui)
                if norm_ui:
                    processed_widgets.append((vend_ui, norm_ui, set(norm_ui.split())))

            for _, row in DF_RENDICIONES_CACHE.iterrows():
                vendedor_sql = str(row['Vendedor']).strip().upper()
                monto = parse_moneda_robusto(row['Efectivo'])

                # Pre-procesar nombre SQL una sola vez por fila
                norm_sql = normalizar_texto(vendedor_sql)
                if not norm_sql:
                    continue
                pal_sql = set(norm_sql.split())

                for vend_ui, norm_ui, pal_ui in processed_widgets:
                    # Lógica inline de son_nombres_similares optimizada
                    match = False

                    # 1. Si DB es subconjunto de UI -> MATCH
                    if pal_sql.issubset(pal_ui):
                        match = True
                    else:
                        # 2. Reglas manuales "ROMAN"
                        if "ROMAN" in norm_sql or "ROMAN" in norm_ui:
                            if "VELAZQUEZ" in norm_sql and "VELAZQUEZ" in norm_ui:
                                match = True
                            # Si tiene ROMAN pero no cumple la regla, es False (no chequeamos subset inverso)

                        # 3. Si UI es subconjunto de DB -> MATCH (solo si no entramos en regla ROMAN)
                        elif pal_ui.issubset(pal_sql):
                            match = True
