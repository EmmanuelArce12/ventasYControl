...
def _fetch_sql_data(d, h):
    """
    Ejecuta consultas SQL en hilo secundario y devuelve diccionarios con dataframes procesados.
    """
    res = {
        "ok": False,
        "error": None,
        "df_efectivo": None,
        "df_qr_raw": None,     # para DATOS_DETALLE_QR
        "df_qr_final": None,   # para totales y descuentos
        "df_fact": None,
        "df_per_sum": None,
        "df_remito": None,
        "df_remito_sum": None,
    ...
    try:
        ...
        # 1. EFECTIVO / RENDICIONES
        res["df_efectivo"] = pd.read_sql(SQL_EFECTIVO, conn, params=[d, h])
        ...
        df_qr_grouped["QR_FINAL"] = (
            df_qr_grouped["IMPORTE"]
            + df_qr_grouped["CASHOUT"]
            - df_qr_grouped["DESC_PROMO_NUM"]
        ).round(2)

        res["df_qr_final"] = df_qr_grouped

        # Preparar descuentos por vendedor (se usará en UI)
        descuentos = {}
        # Aquí necesitamos separar por operario. Como df_qr_grouped tiene todos, lo haremos
        # en el paso de UI o aquí mismo si filtramos.
        # Mejor devolver el DF completo y que la UI lo filtre como antes.
        ...
        res["df_fact"] = df_fact.drop(indices_a_borrar)
        ...
        res["df_remito"] = df_remito
        res["df_remito_sum"] = df_remito.groupby('Nombre_Vendedor')['Total_Remito'].sum().reset_index()

        res["ok"] = True

    except Exception as e:
        res["error"] = str(e)
        print("ERROR EN BACKGROUND THREAD:", e)
    finally:
        if conn:
            try:
                conn.close()
...
def _update_ui_from_sql(data, on_complete=None):
    global DF_GNC_SQL_CACHE, DF_RENDICIONES_CACHE, DATOS_RENDICIONES
    global DATOS_DETALLE_QR, DESCUENTOS_QR_POR_VENDEDOR
    global DATOS_DETALLE_FACTURACION, DATOS_DETALLE_REMITOS
    global DATOS_PRODUCTOS_FACTURADOS

    if not data["ok"]:
        if data["error"]:
            messagebox.showerror("Error SQL", data["error"])
        if on_complete:
            on_complete()
        return
    ...
    try:
        ...
        # 2. ACTUALIZAR DATOS_RENDICIONES
        # Limpiar SQL previo
        ...
        # 3. ACTUALIZAR DATOS_DETALLE_QR
        DATOS_DETALLE_QR.clear()
        df_qr = data["df_qr_final"]
        ...
        # 6. ACTUALIZAR FACTURACION
        DATOS_DETALLE_FACTURACION.clear()
        df_fact = data["df_fact"]
        ...
        # Inyectar totales en widgets (prod)
        ...
        messagebox.showinfo("Proceso completado", "QR y total facturado fueron procesados correctamente.")
...
,"row":row, "fusionado":False,"combo": combo}
    ...
    def ejecutar_consulta_sql_con_planillas(d, h, on_complete=None):
        global DATOS_DETALLE_FACTURACION
        DATOS_DETALLE_FACTURACION = {}

        # 1. Crear ventana de carga (Loading)
        loading = tk.Toplevel(root)
        loading.title("Consultando SQL...")
        ...
        try:
            ...
            x = root.winfo_x() + (root.winfo_width() // 2) - (loading.winfo_width() // 2)
            y = root.winfo_y() + (root.winfo_height() // 2) - (loading.winfo_height() // 2)
            loading.geometry(f"+{x}+{y}")
        except:
            pass

        tk.Label(loading, text="Consultando base de datos...", font=("Arial", 11)).pack(pady=20)
        pb = ttk.Progressbar(loading, mode="indeterminate")
        pb.pack(fill="x", padx=20)
        pb.start(10)

        # 2. Cola para comunicación thread -> main
        q = queue.Queue()

        def worker():
            res = _fetch_sql_data(d, h)
            q.put(res)
        ...
        # 4. Polling para chequear si terminó
        def check_result():
            try:
                res = q.get_nowait()
                # Si llegamos acá, el hilo terminó
                loading.destroy()
                _update_ui_from_sql(res, on_complete)
            except queue.Empty:
