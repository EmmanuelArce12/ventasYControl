...
def encontrar_encabezado_y_columnas(path):
    # Intentar leer solo las primeras 50 filas para optimizar rendimiento
    try:
        raw = pd.read_excel(path, header=None, nrows=50)
    except:
        # Fallback en caso de error (ej: archivo muy corto o error de lectura parcial)
        try:
    ...
    # Función interna para buscar el encabezado
    def buscar_en_df(df):
        # Usar itertuples para mayor velocidad que iterrows
        for row in df.itertuples(index=True):
            # row[0] es el índice
            # row[1:] son los valores de la fila

            # Convertir valores a string y minúsculas para búsqueda robusta
            valores = [str(x).lower() for x in row[1:]]

            # Verificar si contiene "vendedor" y "fecha"
            has_vendedor = any('vendedor' in x for x in valores)
            has_fecha = any('fecha' in x for x in valores)

            if has_vendedor and has_fecha:
                # Buscar columna de importe
                for col_idx, val in enumerate(valores):
                    # Check substring match equivalent to regex 'importe|total|pesos'
                    if 'importe' in val or 'total' in val or 'pesos' in val:
                        return row[0], col_idx

                # Si llegamos aquí es porque encontramos el encabezado PERO NO la columna importe
                messagebox.showerror(
                    "Error de formato",
                    "Se encontró el encabezado, pero no la columna IMPORTE / TOTAL / PESOS."
                )
                return -2, -2 # Código especial para indicar error ya mostrado
        return -1, -1

    h, p = buscar_en_df(raw)
    ...
    # Si leímos solo 50 filas y no encontramos, intentar leer todo el archivo
    # (solo si el archivo original tenía más de 50 filas, de lo contrario ya revisamos todo)
    if len(raw) >= 50:
        try:
            raw = pd.read_excel(path, header=None)
            h, p = buscar_en_df(raw)
            ...
            pass # Fallará abajo con el messagebox
