...
def cargar_rendiciones_sql():
    ...
    # 1. Precomputar sets normalizados para widgets (UI)
    ui_cache = []
    for vend_ui in widgets.keys():
        norm_ui = normalizar_texto(vend_ui)
        if not norm_ui:
            continue
        set_ui = set(norm_ui.split())
        ui_cache.append((vend_ui, norm_ui, set_ui))

    # 2. Cache de resultados de matching: { sql_vendedor_raw: matched_ui_vendor_key or None }
    match_cache = {}

    for _, row in DF_RENDICIONES_CACHE.iterrows():
        raw_vend = row['Vendedor']
        matched_vend_ui = None

        if raw_vend in match_cache:
            matched_vend_ui = match_cache[raw_vend]
        else:
            # Buscar match si no está en cache
            nom_sql = normalizar_texto(raw_vend)

            if nom_sql:
                set_sql = set(nom_sql.split())

                for vend_ui, norm_ui, set_ui in ui_cache:
                    match = False

                    # Lógica optimizada de son_nombres_similares(vend_ui, nom_sql)
                    # excel=vend_ui (norm_ui, set_ui), db=nom_sql (set_sql)

                    # 1. Si DB es subset de Excel -> MATCH
                    if set_sql.issubset(set_ui):
                        match = True
                    # 2. Reglas manuales "ROMAN"
                    elif "ROMAN" in nom_sql or "ROMAN" in norm_ui:
                        if "VELAZQUEZ" in nom_sql and "VELAZQUEZ" in norm_ui:
                            match = True
                        else:
                            match = False
                    # 3. Si Excel es subset de DB -> MATCH (solo si no se aplicó regla ROMAN)
                    elif set_ui.issubset(set_sql):
                        match = True

                    if match:
                        matched_vend_ui = vend_ui
                        break

            match_cache[raw_vend] = matched_vend_ui

        if matched_vend_ui:
            monto = parse_moneda_robusto(row['Monto_Rendido'])
            DATOS_RENDICIONES.setdefault(matched_vend_ui, {}).setdefault(
                'movimientos', []
            ).append({
                'origen': 'sql',
